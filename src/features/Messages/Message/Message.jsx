...
export default function Message(props) {
  ...
  return (
    <div
      className={`Message ${currentUser.uid === thisUser.uid ? "CUIA" : ""}`}
    >
      <div
        className={
          currentUser.uid === thisUser.uid
            ? "current_user_is_author"
            : "other_user_is_author"
        }
      >
        {currentUser.uid === thisUser.uid ? (
          ""
        ) : (
          <Avatar src={thisUser.photoURL} />
        )}
        ...
        <div className="MessageFlex">
          ...
          {props.message.type !== "photo" ? (
            <div
              ...
              title={createDate(props.message.date)}
            >
              {props.message.type === "file" ? (
                <a
                  className="fileMessage"
                  href={props.message.file}
                  target="_blank"
                >
      ...
      <Menu
        ...
        onClose={handleClose}
      >
        <MenuItem onClick={() => deleteMessage(props.message.id, handleClose)}>
          Remove
        </MenuItem>
      </Menu>
    </div>
